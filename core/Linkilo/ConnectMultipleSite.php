...
public static function clear_data_table(){
    ...
    public static function process_tokens(){
        if( isset($_POST['initok']) && 
            ...
            if(empty(get_option('linkilo_link_external_sites', false))){
                die;
            }
        ...
        {
            // if we're not accepting site linking, exit
            if(get_option('linkilo_link_external_sites', false)){
                die;
            }
...
//                'Accept-Encoding: gzip, deflate, br',
    ...
    public static function export_data($page = 0, $limit = 5000){
        ...
        //get data
        $statuses_query = " AND post_status = 'publish' ";  //Linkilo_Build_DatabaseQuery::postStatuses('p');
//        $report_post_ids = Linkilo_Build_DatabaseQuery::reportPostIds();
//        $report_term_ids = Linkilo_Build_DatabaseQuery::reportTermIds();
        ...
        $total_items = Linkilo_Build_UrlRecord::getTotalItems($query);
    ...
    public static function create_initial_request_string($target_url = ''){
        ...
        $current_url = trailingslashit(site_url());
    ...
    public static function process_initial_request_string($string = '', $time = 0){
        ...
        $license_active = Linkilo_Build_ActiveLicense::check_site_license($calling_site);
    ...
    public static function update_secondary_request_hash_data($hash, $target_url, $time, $authenticated = false){
        ...
        update_option('linkilo_stored_request_hashes', $updated_hashes);
    }
    ...
    public static function ajax_register_selected_site(){
        ...
        if(!isset($_POST['url'])){
            wp_send_json(array('error' => array('title' => __('No Url', 'linkilo'), 'text' => __('The site url field is empty, please add the url of the site you want to link to.', 'linkilo'))));
        }else{
            $url = trailingslashit(wp_http_validate_url($_POST['url']));
            if(empty($url)){
                wp_send_json(array('error' => array('title' => __('Url Format Error', 'linkilo'), 'text' => __('The given url was not in the necessary format. Please enter the url as it appears in your browser\'s address bar, including the protocol (https or http).', 'linkilo'))));
            }
        ...
        $license_active = Linkilo_Build_ActiveLicense::check_site_license($url);

        // if it's not valid, tell the user
        if(empty($license_active)){
            wp_send_json(array('error' => array('title' => __('Site License Not Verified', 'linkilo'), 'text' => __('The license for the given site could not be verified. Please check to make sure the site is using the same license key as this site.', 'linkilo'))));
        }*/
        ...
        if($saved){
            $site_link_button = '
            <label>
            <a href="#" class="linkilo-link-site-button button-primary" data-nonce="' . wp_create_nonce(wp_get_current_user()->ID . 'link-site-nonce') . '">' . __('Attempt Site Linking', 'linkilo') . '</a>
            <a href="#" class="linkilo-unregister-site-button button-primary button-purple site-linking-button" data-nonce="' . wp_create_nonce(wp_get_current_user()->ID . 'unregister-site-nonce') . '">' . __('Unregister Site', 'linkilo') . '</a>
            </label>';
            wp_send_json(array('info' => array('title' => __('Site Registered', 'linkilo'), 'text' => __("The external site has been registered on this site, please go to the external site and register this site there. \n\n Attempting to link the sites before they are registered with each other will fail.", 'linkilo'), 'link_button' => $site_link_button)));
        }elseif($saved === false){
            wp_send_json(array('info' => array('title' => __('Site Already Registered', 'linkilo'), 'text' => __('The given site has already been registered on this site.', 'linkilo'))));
        }else{
            wp_send_json(array('error' => array('title' => __('Site Not Registered', 'linkilo'), 'text' => __('Unfortunately, the site could not be registered.', 'linkilo'))));
        }
    }
    ...
    public static function ajax_link_selected_site(){
        ...
        if(!empty($license_active)){
            ...
            if($ping_result){
                ...
                wp_send_json(array('success' => array('title' => __('Site Linked!', 'linkilo'), 'text' => __('The site has been linked to this one and data can be shared between them!', 'linkilo'), 'unlink_button' => $unlink_button, 'import_button' => $import_button, 'suggestions_button' => $suggestions_button)));
            }else{
                // if the ping wasn't successful, tell the user about it
                wp_send_json(array('info' => array('title' => __('Site Not Linked', 'linkilo'), 'text' => __('The site could not be linked to this one. Please check the "Site Interlinking Access Code" on the external site to make sure it exactly matches the code on this site.', 'linkilo'))));
            }
        }else{
            wp_send_json(array('error' => array('title' => __('Site License Not Verified', 'linkilo'), 'text' => __('The license for the given site could not be verified. Please check to make sure the site is using the same license key as this site.', 'linkilo'))));
        }*/
    ...
    public static function ajax_outer_site_suggestion_toggle(){
        ...
        if(!isset($_POST['url']) || empty($_POST['url'])){
            wp_send_json(array('error' => array('title' => __('Url Missing', 'linkilo'), 'text' => __('The site url was missing from the unregister attempt. Please reload the page and try again.', 'linkilo'))));
        }

        $url = wp_http_validate_url($_POST['url']);

        if(empty($url)){
            wp_send_json(array('error' => array('title' => __('Url Format Error', 'linkilo'), 'text' => __('The site url was misformatted. Please reload the page and try again.', 'linkilo'))));
        }
        ...
        if($updated){
            if($current_status){
                wp_send_json(array('success' => array('title' => __('Site Suggestions Disabled!', 'linkilo'), 'text' => __('Linkilo will not show suggestions to this external site.'))));
            }else{
                wp_send_json(array('success' => array('title' => __('Site Suggestions Enabled!', 'linkilo'), 'text' => __('Linkilo will now show suggestions to the external site.'))));
            }
        }else{
            wp_send_json(array('info' => array('title' => __('Site Setting Not Saved', 'linkilo'), 'text' => __('There was an error that prevented Linkilo from saving the option. Please reload the page and try again.'))));
        }
    }
    ...
    public static function ajax_remove_registered_site(){
        ...
        if($removed){
            wp_send_json(array('success' => array('title' => __('Site Unregistered!', 'linkilo'), 'text' => __('The external site has been removed from this site\'s list of registered sites.'))));
        }else{
            wp_send_json(array('info' => array('title' => __('Site Could Not Be Unregistered', 'linkilo'), 'text' => __('The external site could not be removed from the site list. Please reload the page and try again.'))));
        }
    }
    ...
    public static function ajax_remove_linked_site(){
        ...
        if(!isset($_POST['url']) || empty($_POST['url'])){
            wp_send_json(array('error' => array('title' => __('Url Missing', 'linkilo'), 'text' => __('The site url was missing from the unlink attempt. Please reload the page and try again.', 'linkilo'))));
        }
        ...
        if($removed){
            wp_send_json(array('success' => array('title' => __('Site Unlinked!', 'linkilo'), 'text' => __('The external site has been removed from this site\'s list of sites. Just a reminder, the external site may need to have this site removed from its list as well.'))));
        }else{
            wp_send_json(array('info' => array('title' => __('Site Could Not Be Unlinked', 'linkilo'), 'text' => __('The external site could not be removed from the site list. Please reload the page and try again.'))));
        }
    }
    ...
    public static function ajax_download_all_posts(){
        ...
        if(empty($hash_data) || empty($hash_data['authenticated'])){
            ...
            if(empty($auth)){
                wp_send_json(array('error' => array('title' => 'Could Not Authenticate', 'text' => 'Linkilo was not able to complete the authentication process. This can be caused by not having this site registered on the external one, or if the site identification codes don\'t match between the sites.')));
            }
        }
        ...
        if(isset($_POST['reset']) && !empty($_POST['reset'])){
            ...
            if(empty($cleared)){
                wp_send_json(array('error' => array('title' => 'Could Not Clear Existing Data', 'text' => 'Linkilo could not erase the site\'s existing post data from the database. Please reload the page and try again.')));
            }
        }
        ...
        if(!empty($response) && isset($response->data) && isset($response->hmac)){
            ...
            if(self::check_hmac($response->data, $page, $url, $response->hmac)){
                ...
                if(!empty($response->data->found_items) && $response->data->total > 0){
                    ...
                    $status['saved'] += $saved;
                    $status['page']  += 1;
                    $status['message'] = sprintf(__('Posts Imported: %d of %d', 'linkilo'), $status['saved'], $response->data->total);
                    ...
                    wp_send_json(array('success' => array('title' => __('Download complete!', 'linkilo'), 'text' => __('All of the post data has been downloaded and saved.', 'linkilo'))));
                }
            }else{
                wp_send_json(array('error' => array('title' => __('Data Error', 'linkilo'), 'text' => __('There was a problem with the data from the other site. The import could not be run', 'linkilo'))));
            }
        }else{
            wp_send_json(array('error' => array('title' => __('Site did not respond', 'linkilo'), 'text' => __('Linkilo was not able to connect to the other site.', 'linkilo'))));
        }
