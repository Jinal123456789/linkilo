 ...
 * Gutenberg editor
 *
 * Class Linkilo_Build_Editor_Gutenberg
 */
class Linkilo_Build_Editor_Kadence
{
    ...
    {
        $matches = Linkilo_Build_RelateUrlKeyword::findKeywordLinks($keyword, $element);
        if (!empty($matches[0])) {
            if (!$left_one || self::$keyword_links_count) {
                Linkilo_Build_RelateUrlKeyword::removeAllLinks($keyword, $element);
            }
            if($left_one && self::$keyword_links_count == 0 and count($matches[0]) > 1) {
                Linkilo_Build_RelateUrlKeyword::removeNonFirstLinks($keyword, $element);
            }
            self::$keyword_links_count += count($matches[0]);
            $element = Linkilo_Build_WordFunctions::replaceUnicodeCharacters($element, true);
        }
    }
    ...
    {
        if (Linkilo_Build_UrlReplace::hasUrl($element, $url)) {
            Linkilo_Build_UrlReplace::replaceLink($element, $url);
        }
    }
    ...
    {
        preg_match('`data-linkilo=\"url\" (href|url)=[\'\"]' . preg_quote($url->new, '`') . '\/*[\'\"]`i', $element, $matches);
        if (!empty($matches)) {
            Linkilo_Build_UrlReplace::revertURL($element, $url);
        }
    }
}