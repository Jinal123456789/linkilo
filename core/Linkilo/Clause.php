...
class Linkilo_Build_Clause
{
    ...
    public static function TitleKeywordsCheck(&$phrases, $keyword)
    {
        foreach ($phrases as $phrase) {
            $keyword_words = Linkilo_Build_WordFunctions::getStemmedWords($keyword);
            $suggestion_words = Linkilo_Build_WordFunctions::getStemmedWords($phrase->suggestions[0]->post->getTitle());
            $common_words = 0;
    ...
    /**
     * Sort incoming post suggestions by post score DESC
     *
     * @param $phrases
     */
    public static function IncomingSort(&$phrases)
    {
        if(!empty($phrases)){
