...
class Linkilo_Build_SetExcelData
{
    ...
    public static function exportPost($post)
    {
        ...
        $sheet->setCellValue('A1', 'Title')
            ...
            ->setCellValue('A3', 'URL')
            ->setCellValue('A5', 'Incoming Inner URLs' . (!empty($incoming_internal) ? ' (' . count($incoming_internal) . ')' : ''))
            ->setCellValue('D5', 'Outgoing Inner URLs' . (!empty($outgoing_internal) ? ' (' . count($outgoing_internal) . ')' : ''))
            ->setCellValue('G5', 'Outgoing Outer URLs' . (!empty($outgoing_external) ? ' (' . count($outgoing_external) . ')' : ''))
